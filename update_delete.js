...
// 2. Replace Header conditional to be ONLY on the <p> tag just like ExtractPages
const headerRegex = /(<h2 className="text-2xl sm:text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Delete PDF Pages<\/h2>\s*)(<p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 font-medium max-w-md mx-auto leading-relaxed">Pick pages to discard and download the trimmed document instantly\.<\/p>)/;
code = code.replace(headerRegex, `$1{!file && $2}`);

// 3. Replace the Dropzone area
const dropzoneRegex = /\{\!file && \!loading && \([\s\S]*?Choose Document\s*<\/button>\s*<\/div>\s*\)\}/;

const newDropzone = `{!file && !loading && (
...
code = code.replace(dropzoneRegex, newDropzone);
