...
if (!code.includes('Instant", desc: "Lightning fast processing"')) {
...
// Check if we already have 2 copies of the 3-item grid to avoid duplicating
const instantMatches = [...code.matchAll(/title: "Instant"/g)];
if (instantMatches.length === 1) { // Only the first one was added above
    code = code.replace(afterUploadTopRegex, `$1\n${gridAfterUpload}`);
}

// 6. Add 4-item Universal Feature Grid to the bottom of the card, CENTERED!
const universalGrid = `
...
if (!code.includes('Universal Feature Grid')) {
    code = code.replace(/(\s*<\/div>\s*<\/div>\s*<\/div>\s*\);)/, `${universalGrid}$1`);
}

// 7. Auto-expand settings on upload
if (!code.includes('setShowSettings(true);')) {
    code = code.replace(
      'setResult(null);\n    try {',
      'setResult(null);\n    setShowSettings(true);\n    try {'
    );
    // Try the CRLF version if it didn't match
    if (!code.includes('setShowSettings(true);')) {
        code = code.replace(
            'setResult(null);\r\n    try {',
            'setResult(null);\r\n    setShowSettings(true);\r\n    try {'
...
if (code.includes(oldButtons)) {
    ...
    // Try regex if whitespace varies
