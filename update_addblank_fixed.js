...
// 1. Add missing imports to lucide-react ONLY!
...
// 3. Hide Sidebar before upload. Use a precise regex!
code = code.replace(
  /<div\s+className=\{`w-full lg:w-\[300px\] bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl h-fit lg:sticky lg:top-4 overflow-hidden flex-shrink-0`\}\s*>/,
  '<div className={`w-full lg:w-[300px] bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl h-fit lg:sticky lg:top-4 overflow-hidden flex-shrink-0 ${!file ? \\'hidden\\' : \\'\\'}`}>'
);
