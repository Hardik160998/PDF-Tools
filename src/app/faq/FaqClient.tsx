    ...
    <div className="min-h-screen">
      ...
      <section className="container mx-auto px-4 pt-16 pb-10 text-center max-w-3xl">
        ...
        <p className="text-lg text-slate-500 mb-2">
          {totalFaqs} answers covering everything about SmartPDFs Pro.
        </p>
      </section>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-56 shrink-0">
            ...
            <div className="hidden lg:flex bg-white rounded-2xl border border-slate-100 shadow-sm p-2 flex-col gap-1">
              {CATEGORIES.map(({ id, label, icon: Icon, color, faqs }) => (
                ...
                >
                  <div
                    className={`w-6 h-6 ${activeCategory === id ? "bg-white/20" : color} rounded-lg flex items-center justify-center shrink-0`}
                  >
                    <Icon
                      size={13}
                      className={
                        activeCategory === id ? "text-white" : "text-white"
                      }
                    />
                  </div>
          ...
          <div className="flex-1 space-y-2">
            <div className="flex items-center gap-3 mb-4">
              <div
                className={`w-8 h-8 ${currentCategory.color} rounded-xl flex items-center justify-center text-white`}
              >
            ...
            {currentCategory.faqs.map((faq, i) => (
              ...
              >
                ...
                >
                  <span
                    className={`font-bold text-sm leading-snug ${openIndex === i ? "text-red-500" : "text-slate-900"}`}
                  >
                    {faq.q}
                  </span>
                  <ChevronDown
