...
import { Type, Upload, FileSearch, Download, FileJson, ImageIcon, FileText, FileSpreadsheet, Presentation, CheckCircle } from 'lucide-react';
import { CenteredCardSkeleton } from '../[id]/skeletons';
import { HowItWorksShimmer, RelatedToolsShimmer, usePageMounted } from '../_shimmer';

const ExtractText = dynamic(() => import('@/components/tools/ExtractText'), { ssr: false, loading: () => <CenteredCardSkeleton accent="rgb(191 219 254)" /> });

const FEATURES = ['Extracts all text instantly','View as plain text or XML','Copy to clipboard in one click','Download as .txt or .xml','Preserves page structure','100% private — runs in browser'];
const STEPS = [
...
const ACCENT = 'rgba(59,130,246,0.15)';
const HOVER = 'text-blue-500';
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #eff6ff33 0%, #fff0 50%, #dbeafe33 100%)' }}>
      <section className="pt-16 pb-8 text-center">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 text-blue-600 border border-blue-100 text-xs font-black uppercase tracking-widest shadow-sm mb-6"><Type size={13} /> Free PDF to Text Tool</div>
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight leading-tight mb-5">Extract Text From PDF{' '}<span style={{ background: 'linear-gradient(135deg, #3182ce, #1e3a8a)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Instantly</span></h1>
          <p className="text-lg text-slate-500 font-medium leading-relaxed max-w-2xl mx-auto mb-8">Pull all text content out of any PDF in seconds. View it as plain text or structured XML, copy to clipboard, or download — all processed locally in your browser.</p>
          <div className="flex flex-wrap justify-center gap-2 mb-4">{FEATURES.map(f => <span key={f} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-100 rounded-full text-xs font-bold text-slate-600 shadow-sm"><CheckCircle size={12} className="text-blue-500" /> {f}</span>)}</div>
        </div>
      </section>
      <section className="pb-8"><div className="container mx-auto px-4 max-w-7xl"><ExtractText id="extract-text" /></div></section>
      {!mounted ? <HowItWorksShimmer accent={ACCENT} /> : (
        ...
        <section className="py-16"><div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{RELATED.map(t => (
            <a key={t.id} href={`/tool/${t.id}`} className="group bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col gap-4">
              ...
              <div><h3 className={`text-base font-black text-slate-900 mb-1 group-hover:${HOVER} transition-colors`}>{t.title}</h3><p className="text-sm text-slate-500 font-medium leading-relaxed">{t.description}</p></div>
              <div className={`mt-auto pt-2 text-xs font-bold ${HOVER} flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity`}>Open tool &#8594;</div>
            </a>
          ))}</div>
        </div></section>
      )}
