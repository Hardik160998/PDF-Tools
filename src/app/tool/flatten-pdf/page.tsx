...
function FlattenPdfSkeleton() {
  return (
    <div className="max-w-3xl mx-auto py-4 sm:py-12 px-2 sm:px-4 text-center animate-pulse">
      {/* Dynamic SEO Schemas */}
      {(() => {
        const meta = getToolMeta("flatten-pdf");
        return meta ? (
          <>
            <WebAppSchema
              name={`${meta.title} – Free Online Tool`}
              description={meta.description}
              url={getToolUrl("flatten-pdf")}
            />
            {meta.faqs.length > 0 && <FAQSchema faqs={meta.faqs} />}
            <BreadcrumbSchema
              items={[
                { label: "Tools", href: "/#tools" },
                { label: meta.title, href: `/tool/flatten-pdf` },
              ]}
            />
          </>
        ) : null;
...
export default function FlattenPdfPage() {
              ...
              <div className="space-y-4">
                {[
                  ...
                  },
                  {
                    ...
                    a: "Yes, absolutely. Like all tools on SmartPDFs Pro, the Flatten PDF process is run 100% locally in your web browser. Your document is processed in local memory and is never uploaded to any cloud server, ensuring full data privacy.",
                  },
                  {
  ...
  <span className="p-1.5 rounded-full bg-violet-500/10 text-violet-500 shrink-0">
    <HelpCircle size={18} />
  </span>
  <h3 className="text-base sm:text-base font-bold text-slate-800 dark:text-white uppercase tracking-tight group-hover:text-violet-500 transition-colors">
                        {item.q}
                      </h3>
</div>
