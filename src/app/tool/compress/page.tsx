    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950">
      <div className="max-w-7xl mx-auto px-4 pt-10 sm:pt-16 pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-16 mb-16">
          {[
            { title: "Smart Compression", desc: "Optimizes internal PDF object streams and cross-reference tables without re-rendering pages.", gradient: "linear-gradient(135deg,#10b981,#059669)" },
            { title: "Preserve Quality", desc: "Ideally suited for documents with lots of text or vectors where image re-encoding isn't needed.", gradient: "linear-gradient(135deg,#10b981,#059669)" },
            { title: "Privacy First", desc: "Your data stays on your device. Secure, local, and incredibly fast processing.", gradient: "linear-gradient(135deg,#22c55e,#15803d)" }
          ].map((feat, i) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {RELATED.map(t => (
            <a key={t.id} href={`/tool/${t.id}`} className="group bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col gap-4 text-left">
              ...
              <div><h3 className="text-base font-black text-slate-900 dark:text-white mb-1 group-hover:text-emerald-500 transition-colors">{t.title}</h3><p className="text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed">{t.description}</p></div>
              <div className="mt-auto pt-2 text-xs font-bold text-emerald-500 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">Open tool &#8594;</div>
            </a>
          ))}
        </div>
      </div>
