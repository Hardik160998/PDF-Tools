...
export default function WebpToPngPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #f0fdfa33 0%, #fff0 50%, #ccfbf133 100%)' }}>
      <section className="pt-16 pb-8 text-center">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-teal-50 text-teal-600 border border-teal-100 text-xs font-black uppercase tracking-widest shadow-sm mb-6"><ImageIcon size={13} /> Free WebP to PNG Converter</div>
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight leading-tight mb-5">
            Convert WebP to PNG{' '}
            <span style={{ background: 'linear-gradient(135deg, #14b8a6, #0f766e)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Instantly</span>
          </h1>
          <p className="text-lg text-slate-500 font-medium leading-relaxed max-w-2xl mx-auto mb-8">
            Convert WebP images to lossless PNG format in one click. Perfect for editing workflows, design tools, and platforms that require PNG with full transparency support.
          </p>
          <div className="flex flex-wrap justify-center gap-2 mb-4">
            {FEATURES.map(f => (
              <span key={f} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-100 rounded-full text-xs font-bold text-slate-600 shadow-sm">
                <CheckCircle size={12} className="text-teal-500" /> {f}
              </span>
            ))}
          </div>
        </div>
