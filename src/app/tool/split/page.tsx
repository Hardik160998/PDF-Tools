...
import { Scissors, Upload, Settings, Download, Combine, LayoutGrid, Zap, FileSymlink, Lock, CheckCircle } from 'lucide-react';
...
const STEPS = [
  { icon: Upload,   title: 'Upload Your PDF',    desc: 'Select the PDF file you want to split. Any size works — processed entirely in your browser.' },
  { icon: Settings, title: 'Choose Split Mode',  desc: 'Divide into 2, 3, or 4 equal parts — or extract every single page as its own PDF file.' },
  { icon: Download, title: 'Download ZIP',        desc: 'Click "Split PDF" and download a ZIP archive containing all your split PDF files instantly.' },
];

const RELATED_TOOLS = [
  { id: 'merge',        title: 'Merge PDF',      description: 'Combine multiple PDF files back into one document in the order you choose.',    icon: Combine,     gradient: 'linear-gradient(135deg, #f26522, #c2410c)', shadow: 'rgba(242,101,34,0.3)',  tag: 'Organize' },
  ...
  { id: 'protect',      title: 'Protect PDF',    description: 'Encrypt your split PDFs with a password to keep them secure.',                  icon: Lock,        gradient: 'linear-gradient(135deg, #ef4444, #b91c1c)', shadow: 'rgba(239,68,68,0.3)',   tag: 'Security' },
      ...
      <section className="pt-16 pb-8 text-center">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-50 text-orange-600 border border-orange-100 text-xs font-black uppercase tracking-widest shadow-sm mb-6">
            <Scissors size={13} /> Free PDF Split Tool
          </div>
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight leading-tight mb-5">
            Split PDF Into{' '}
            <span style={{ background: 'linear-gradient(135deg, #f26522, #f59e0b)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Separate Files</span>
          </h1>
          <p className="text-lg text-slate-500 font-medium leading-relaxed max-w-2xl mx-auto mb-8">
            Divide any PDF into equal parts or extract every page as its own file. Choose your split mode, click once, and download a ZIP — all in your browser.
          </p>
