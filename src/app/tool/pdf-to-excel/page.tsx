...
import { FileSpreadsheet, Upload, Sparkles, Download, FileText, Presentation, Type, Zap, Lock, CheckCircle } from 'lucide-react';
...
const FEATURES = ['Extracts tables to XLSX','Preserves rows & columns','Multiple sheets supported','Fast cloud conversion','Files deleted within 1 hour','Encrypted HTTPS transfer'];
const STEPS = [
  { icon: Upload,   title: 'Upload Your PDF',    desc: 'Select the PDF containing tables or data. It is securely uploaded over HTTPS for cloud conversion.' },
  { icon: Sparkles, title: 'Extract to Excel',   desc: 'Our engine detects tables and data structures and recreates them as editable XLSX spreadsheet rows.' },
  { icon: Download, title: 'Download XLSX',      desc: 'Your Excel file is ready to download and edit. The uploaded PDF is permanently deleted within 1 hour.' },
];
const RELATED = [
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #f0fdf433 0%, #fff0 50%, #dcfce733 100%)' }}>
      <section className="pt-16 pb-8 text-center">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-green-50 text-green-600 border border-green-100 text-xs font-black uppercase tracking-widest shadow-sm mb-6"><FileSpreadsheet size={13} /> Free PDF to Excel Tool</div>
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight leading-tight mb-5">Convert PDF to Excel{' '}<span style={{ background: 'linear-gradient(135deg, #22c55e, #15803d)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Editable XLSX</span></h1>
          <p className="text-lg text-slate-500 font-medium leading-relaxed max-w-2xl mx-auto mb-8">Extract tables and data from any PDF into a fully editable Excel spreadsheet. Rows, columns, and cell data are preserved with high accuracy.</p>
      ...
      <section className="pb-8"><div className="container mx-auto px-4 max-w-7xl"><OfficeTools id="pdf-to-excel" /></div></section>
