...
const EditTools = dynamic(() => import('@/components/tools/EditTools'), { ssr: false, loading: () => <CenteredCardSkeleton accent="rgb(203 213 225)" /> });

const FEATURES = ['Edit Title, Author & Subject','Update Keywords & Creator','Modify creation & modified dates','View existing metadata first','Instant download','100% private — runs in browser'];
const STEPS = [
...
const RELATED = [
  ...
  { id: 'compress',     title: 'Compress PDF',   description: 'Reduce PDF file size while keeping quality sharp and text crisp.',                 icon: Zap,     gradient: 'linear-gradient(135deg, #22c55e, #15803d)', shadow: 'rgba(34,197,94,0.3)',   tag: 'Optimize', href: '/tool/compress'     },
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #f8fafc33 0%, #fff0 50%, #f1f5f933 100%)' }}>
      <section className="pt-16 pb-8 text-center">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 text-slate-600 border border-slate-200 text-xs font-black uppercase tracking-widest shadow-sm mb-6"><Settings size={13} /> Free PDF Metadata Editor</div>
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight leading-tight mb-5">Edit PDF Metadata{' '}<span style={{ background: 'linear-gradient(135deg, #64748b, #334155)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Instantly</span></h1>
          <p className="text-lg text-slate-500 font-medium leading-relaxed max-w-2xl mx-auto mb-8">View and edit hidden PDF metadata — Title, Author, Subject, Keywords, Creator, and dates. Existing values are pre-filled so you can update only what you need.</p>
          <div className="flex flex-wrap justify-center gap-2 mb-4">{FEATURES.map(f => <span key={f} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-100 rounded-full text-xs font-bold text-slate-600 shadow-sm"><CheckCircle size={12} className="text-slate-500" /> {f}</span>)}</div>
        </div>
      </section>
      <section className="pb-8"><div className="container mx-auto px-4 max-w-7xl"><EditTools id="metadata" /></div></section>
      {!mounted ? <HowItWorksShimmer accent="rgba(100,116,139,0.15)" /> : (
        <section className="py-16 bg-white/60"><div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{STEPS.map((s, i) => (
            <div key={i} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 text-center">
              <div className="inline-flex p-4 rounded-2xl bg-slate-100 text-slate-500 mb-4"><s.icon size={28} /></div>
        ...
        <section className="py-16"><div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{RELATED.map(t => (
            <a key={t.id} href={t.href} className="group bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col gap-4">
              ...
              <div><h3 className="text-base font-black text-slate-900 mb-1 group-hover:text-slate-600 transition-colors">{t.title}</h3><p className="text-sm text-slate-500 font-medium leading-relaxed">{t.description}</p></div>
