...
import {
  ScanText, Upload, Download, CheckCircle,
  FileText, EyeOff, Type, FileSearch,
} from "lucide-react";
...
const STEPS = [
  {
    ...
    desc: "Drop any PDF — scanned, image-based, or one with an existing text layer. Nothing leaves your device.",
  },
  {
    icon: ScanText,
    title: "Choose a Mode",
    desc: "Pick 'Make Selectable' to OCR-scan and add a text layer, or 'Remove OCR' to flatten pages back to pure images.",
  },
  {
    icon: Download,
    title: "Download Result",
    desc: "Get your processed PDF instantly. Text is now selectable & searchable — or fully stripped, depending on your choice.",
  },
];
...
const RELATED = [
  {
    id: "extract-text",
    title: "Extract Text",
    description: "Pull all text out of a PDF as plain text or XML.",
    icon: Type,
  ...
  },
  {
    id: "pdf-to-word",
    title: "PDF to Word",
    description: "Convert your PDF into an editable DOCX file.",
    icon: FileText,
    gradient: "linear-gradient(135deg, #3b82f6, #1e3a8a)",
    shadow: "rgba(59,130,246,0.3)",
    tag: "Convert",
...
export default function OcrPdfPage() {
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(135deg, #eff6ff33 0%, #fff0 50%, #dbeafe33 100%)" }}
    >
      {/* ── Hero ── */}
      <section className="pt-16 pb-8 text-center">
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <p className="text-lg text-slate-500 font-medium leading-relaxed max-w-2xl mx-auto mb-8">
            Add a searchable text layer to scanned PDFs — or strip it out entirely. Two modes, one tool, fully in your browser.
          </p>
          <div className="flex flex-wrap justify-center gap-2 mb-4">
            {FEATURES.map(f => (
              <span
                key={f}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-100 rounded-full text-xs font-bold text-slate-600 shadow-sm"
              >
      ...
      {/* ── How it works ── */}
      {!mounted ? (
        <HowItWorksShimmer accent={ACCENT} />
      ) : (
          ...
          <div className="container mx-auto px-4 max-w-4xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {STEPS.map((s, i) => (
                ...
                >
                  <div className="inline-flex p-4 rounded-2xl bg-blue-50 text-blue-500 mb-4">
                    <s.icon size={28} />
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {RELATED.map(t => (
                ...
                >
                  ...
                  <div>
                    <h3 className="text-sm font-black text-slate-900 mb-1 group-hover:text-blue-500 transition-colors">
                      {t.title}
                    </h3>
