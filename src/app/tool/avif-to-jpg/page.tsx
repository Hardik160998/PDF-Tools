...
export default function AvifToJpgPage() {
    return (
        <main className="min-h-screen bg-slate-50/50 dark:bg-slate-950">
            ...
            {/* Dynamic SEO Schemas */}
            {(() => {
                const meta = getToolMeta('avif-to-jpg');
                return meta ? (
                    <>
                        <WebAppSchema name={`${meta.title} – Free Online Tool`} description={meta.description} url={getToolUrl('avif-to-jpg')} />
                        {meta.faqs.length > 0 && <FAQSchema faqs={meta.faqs} />}
                        <BreadcrumbSchema items={[{ label: 'Tools', href: '/#tools' }, { label: meta.title, href: `/tool/avif-to-jpg` }]} />
                    </>
                ) : null;
            })()}
  ...
  <span className="p-1.5 rounded-full bg-pink-500/10 text-pink-500 shrink-0">
    <HelpCircle size={18} />
  </span>
  <h3 className="text-base sm:text-base font-bold text-slate-800 dark:text-white uppercase tracking-tight group-hover:text-pink-500 transition-colors">
                                                {item.q}
                                            </h3>
</div>
