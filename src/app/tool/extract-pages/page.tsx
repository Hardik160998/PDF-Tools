...
import { Layers, Upload, Download, CheckCircle, Scissors, Combine, Zap, FileSymlink, RotateCw } from "lucide-react";
...
const FEATURES = [
  "Visual page thumbnail picker",
  "Select by range (e.g. 1,3,5-8)",
...
const STEPS = [
  { icon: Upload,   title: "Upload Your PDF",   desc: "Select any PDF file. All pages load instantly as visual thumbnails in your browser." },
  { icon: Layers,   title: "Pick Your Pages",   desc: "Click thumbnails to select pages, or type a range like 1,3,5-8. Mix and match freely." },
  { icon: Download, title: "Download New PDF",  desc: "Click Extract and download a brand-new PDF containing only your chosen pages." },
];
...
const RELATED_TOOLS = [
  ...
  { id: "pdf-to-jpg",title: "PDF to JPG",     description: "Convert every PDF page into a high-quality JPG image instantly.",            icon: RotateCw,   gradient: "linear-gradient(135deg,#eab308,#a16207)", shadow: "rgba(234,179,8,0.3)",  tag: "Convert"  },
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(135deg,#fff7ed33 0%,#fff0 50%,#fef3c733 100%)" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {STEPS.map((step, i) => (
                <div key={i} className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 text-center">
                  <div className="inline-flex p-4 rounded-2xl bg-orange-50 dark:bg-orange-500/10 text-orange-500 mb-4"><step.icon size={28} /></div>
      ...
      {!mounted ? <RelatedToolsShimmer /> : (
        <section className="py-16">
          <div className="container mx-auto px-4 max-w-5xl">
            <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight mb-8 text-center">More Organize &amp; Convert Tools</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
