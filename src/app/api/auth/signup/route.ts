...
export async function POST(request: Request) {
  try {
    ...
    if (!supabaseUrl) {
      return NextResponse.json(
        { error: "Supabase URL configuration is missing." },
        { status: 500 },
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "SUPABASE_SERVICE_ROLE_KEY is not defined in .env.local. Please copy your Service Role Key to .env.local to enable signup verification.",
        },
        { status: 450 },
      );
    }
    ...
    await transporter.sendMail({
      from: `"SmartPDFs Pro" <${process.env.GMAIL_USER}>`,
      to: emailLower,
 ...
 Thank you for signing up for SmartPDFs Pro! Please enter the 6-digit verification code below on the signup page to confirm your account.
 </p>
 ...
 <p style="margin: 0 0 4px;">&copy; ${new Date().getFullYear()} SmartPDFs Pro. All rights reserved.</p>
