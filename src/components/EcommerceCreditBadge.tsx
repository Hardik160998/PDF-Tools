  ...
  return (
    <div className="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm max-w-3xl mx-auto">
        ...
        <div className="text-left">
          ...
          <p className="text-sm font-bold text-slate-800 dark:text-white">
            {isPremium ? (
              ...
              <span>
                Remaining: <span className="text-red-500 font-black text-base">{credits}</span> / 10 free credits
              </span>
            )}
          </p>
        </div>
      ...
      {!isPremium && (
        <Link
          href="/premium-plans"
          className="w-full sm:w-auto inline-flex items-center justify-center bg-[#f26522] hover:bg-[#d4541a] text-white text-xs font-black uppercase tracking-wider px-5 py-3 rounded-xl shadow-md transition-all whitespace-nowrap"
        >
          Upgrade to Premium
        </Link>
      )}
