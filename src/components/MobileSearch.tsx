...
import { Search, X, Combine, Scissors, Zap, LifeBuoy, Type, FileJson, ImageIcon, FileText, Presentation, FileSpreadsheet, Globe, Stamp, FileDigit, Settings, Unlock, Lock, Wand2, FileSymlink } from "lucide-react";
      ...
      {open && (
        <div className="mob-search-overlay">
          ...
          <div className="mob-search-header">
            ...
            <button onClick={close} className="mob-search-close">
              Cancel
            </button>
          </div>
          ...
          <div className="mob-search-results">
            {results.length > 0 ? (
              <>
                {!query.trim() && (
                  <p className="mob-search-hint">All tools</p>
                )}
                {results.map(({ id, label, category, icon: Icon }) => (
                  <a
                    key={id}
                    href={`/tool/${id}`}
