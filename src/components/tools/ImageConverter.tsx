...
export default function ImageConverter({ id }: { id: string }) {
  ...
  return (<div className="max-w-4xl mx-auto py-4 sm:py-12 px-0 sm:px-4 text-center">
    <div className="bg-white dark:bg-slate-800 rounded-[1.2rem] sm:rounded-[2.5rem] p-3 sm:p-12 border border-slate-100 dark:border-slate-700 shadow-2xl space-y-6 sm:space-y-8">
      {/* Header */}
      <div className="space-y-2 sm:space-y-3">
        <div className={`inline-flex p-3 sm:p-5 rounded-xl sm:rounded-3xl ${accent.bg} text-white shadow-lg`}>
          <ImageIcon size={28} className="sm:w-[36px] sm:h-[36px]" />
        </div>
        ...
        <p className="text-xs sm:text-base text-slate-500 font-medium px-2">Fast, visual, and secure image conversion.</p>
      </div>

      {results.length === 0 ? (
        <div className="space-y-4 sm:space-y-6">
          {/* Drop zone */}
          <div className={`relative border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl sm:rounded-3xl p-6 sm:p-16 group ${accent.hover} transition-all cursor-pointer bg-slate-50/50 dark:bg-slate-900/50`}>
            <input
              ref={inputRef}
              type="file"
            ...
            <div className="space-y-3 sm:space-y-4 pointer-events-none">
              <div className={`p-4 sm:p-6 bg-white dark:bg-slate-800 rounded-xl sm:rounded-2xl shadow-xl inline-block ${accent.text} group-hover:scale-110 transition-transform`}>
          ...
          {files.length > 0 && (
            <div className="space-y-4 sm:space-y-6 animate-in slide-in-from-bottom-4 duration-500">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                {files.map((file, i) => (
                  <div key={i} className="relative p-2 sm:p-3 bg-slate-50 dark:bg-slate-700 rounded-xl sm:rounded-2xl border border-slate-100 dark:border-slate-600">
                    <div className="aspect-square bg-white dark:bg-slate-800 rounded-lg flex items-center justify-center text-slate-300 overflow-hidden">
                      {previews[i] ? <img src={previews[i]} alt={file.name} className="w-full h-full object-contain" /> : <FileText size={24} className="sm:w-[28px] sm:h-[28px]" />}
                    </div>
                    <p className="text-[10px] sm:text-xs font-bold mt-2 truncate text-slate-500 text-center px-1">{file.name}</p>
                    {/* Always visible remove button */}
                    <button
                      onClick={() => removeFile(i)}
                      className="absolute -top-1.5 -right-1.5 w-5 h-5 sm:w-6 sm:h-6 bg-red-500 text-white rounded-full shadow-lg flex items-center justify-center"
                    >
              ...
              <button
                ...
                disabled={processing}
                className={`w-full py-4 sm:py-5 ${accent.bg} hover:opacity-90 text-white rounded-xl sm:rounded-2xl text-base sm:text-2xl font-black shadow-xl ${accent.shadow} flex items-center justify-center gap-3 sm:gap-4 transition-all disabled:opacity-50`}
              >
                {processing ? <Loader2 className="animate-spin" /> : <Sparkles size={20} className="sm:w-[24px] sm:h-[24px]" />}
        ...
        <div className="space-y-8 animate-in zoom-in duration-700">
          ...
          {id === 'jpg-to-pdf' ? (
            <a
              ...
              className={`w-full py-5 ${accent.bg} hover:opacity-90 text-white rounded-2xl text-xl sm:text-2xl font-black shadow-xl flex items-center justify-center gap-4 transition-all`}
            >
              ...
              {results.length === 1 ? (
                <>
                  <a
                    ...
                    className={`w-full py-4 sm:py-5 ${accent.bg} hover:opacity-90 text-white rounded-xl sm:rounded-2xl text-lg sm:text-2xl font-black shadow-xl flex items-center justify-center gap-4 transition-all`}
                  >
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                    {results.map((url, i) => (
                      <div key={i} className="bg-slate-50 dark:bg-slate-900/50 p-2 sm:p-3 rounded-xl sm:rounded-2xl border border-slate-100 dark:border-slate-800 space-y-2">
                        <div className="aspect-square bg-white dark:bg-slate-800 rounded-lg sm:rounded-xl shadow-sm overflow-hidden flex items-center justify-center">
                          <img src={url} alt={files[i]?.name} className="w-full h-full object-contain" />
                        </div>
                        <a
                          href={url}
                          download={`${files[i]?.name.replace(/\.[^.]+$/, '') || `image_${i + 1}`}.${getTargetExt()}`}
                          className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-black flex items-center justify-center gap-1.5 transition-all shadow-sm"
                        >
