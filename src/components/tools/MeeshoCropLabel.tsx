...
async function findTaxInvoiceY(page: pdfjsLib.PDFPageProxy): Promise<number | null> {
  const content = await page.getTextContent();
  const items = content.items as any[];
  for (const item of items) {
    const text = item.str?.trim().toUpperCase();
    if (text === 'TAX INVOICE' || text?.includes('TAX INVOICE')) {
...
async function extractPageMetadata(page: pdfjsLib.PDFPageProxy): Promise<{ courierName: string; sellerName: string; qty: number; pincode: string; orderId: string; awb: string; customerName: string }> {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    // Enhanced courier detection - check for all variations
...
async function renderPageCroppedAboveTaxInvoice(page: pdfjsLib.PDFPageProxy, scale: number, taxInvoiceY: number | null): Promise<HTMLCanvasElement> {
  ...
  const cropStartY = 10;
  const croppedHeight = Math.max(1, cropEndY - cropStartY);

  const out = document.createElement('canvas');
  out.width = fullCanvas.width;
  out.height = croppedHeight;
  out.getContext('2d')!.drawImage(fullCanvas, 0, cropStartY, fullCanvas.width, croppedHeight, 0, 0, fullCanvas.width, croppedHeight);
  return out;
}
        ...
        (pdfUrl || pdfUrls.length > 0) && (
          <div className="space-y-8 sm:space-y-12 animate-in zoom-in duration-700">
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-sm">TAX INVOICE removed. Clean shipping labels packed into PDF{pdfUrls.length > 1 ? 's' : ''}.</p>
            </div>
            <div className="space-y-4">
              {pdfUrl && <a href={pdfUrl} download="meesho_crop_labels.pdf" className="block py-4 sm:py-5 bg-[#f26522] hover:bg-[#d4541a] text-white rounded-2xl text-xl sm:text-2xl font-black shadow-xl flex items-center justify-center gap-3"><Download size={24} /> Download PDF</a>}
              {pdfUrls.length > 0 && (
