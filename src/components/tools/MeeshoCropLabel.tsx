...
// Find Y coordinate of "TAX INVOICE" text — returns PDF units from bottom
async function findTaxInvoiceY(page: pdfjsLib.PDFPageProxy): Promise<number | null> {
  ...
  for (const item of items) {
    ...
    if (text === 'TAX INVOICE' || text?.includes('TAX INVOICE')) {
      return item.transform[5]; // Y from bottom in PDF units
    }
  }
  return null;
}

// Render page and crop from TOP down to just ABOVE "TAX INVOICE" line
async function renderPageCroppedAboveTaxInvoice(
  ...
  // Add more padding (25px) to ensure TAX INVOICE text is completely excluded
  const cropEndY = Math.max(1, Math.floor(viewport.height - taxInvoiceY * scale) - 25);
  
  // Crop slightly from top (10px) to remove any header margins
  const cropStartY = 10;
  ...
  out.getContext('2d')!.drawImage(
    fullCanvas,
    0, cropStartY, fullCanvas.width, croppedHeight,
    0, 0, fullCanvas.width, croppedHeight
  );
  return out;
}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[1.5rem] sm:rounded-[2.5rem] p-5 sm:p-12 border border-slate-100 dark:border-slate-700 shadow-2xl text-center space-y-6 sm:space-y-10">
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 font-medium">
            Automatically crops Meesho shipping labels — keeps shipping address, return address &amp; barcodes. Removes TAX INVOICE and billing info.
          </p>
        </div>
            ...
            <div className="space-y-8 sm:space-y-12 animate-in zoom-in duration-700">
              ...
              <div className="space-y-4">
                ...
                <p className="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-sm">
                  TAX INVOICE removed. Clean shipping labels packed into one PDF.
                </p>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <a
                  href={pdfUrl}
                  download="meesho_crop_labels.pdf"
