      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div className="w-full lg:w-[320px] bg-white dark:bg-slate-900 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden h-fit lg:sticky lg:top-4 flex-shrink-0">
          ...
          <div className={`${showSettings ? 'block' : 'hidden'} lg:block p-8`}>
            <div className="flex items-center justify-between mb-8">
              <h3 className="hidden lg:block text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">Status</h3>
              ...
              <div className="pt-8 border-t border-slate-50 dark:border-slate-800">
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 mb-6">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Processing Queue</span>
                  </div>
                ...
                >
                  {processing ? (
                    ...
                    <span className="flex items-center justify-center gap-3">Shrink All <Zap size={24} /></span>
                  )}
                </button>
              </div>
            </div>
        ...
        <div className="flex-1 w-full space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-2xl p-6 sm:p-12 min-h-[650px] flex flex-col relative overflow-hidden">
            ...
            {files.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[40px] p-10 sm:p-20 hover:border-emerald-400 cursor-pointer transition-all bg-slate-50/30 dark:bg-slate-900/30 group relative overflow-hidden"
                ...
                <p className="text-slate-400 text-sm mt-4 font-bold tracking-tight text-center relative z-10 uppercase tracking-widest">
                  Batch process documents securely · zero server lag
                </p>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="grid grid-cols-1 gap-4">
                  {files.map((f) => (
                    <div key={f.id} className="flex flex-col sm:flex-row items-center justify-between p-6 bg-slate-50 dark:bg-slate-800 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm gap-6 group hover:shadow-xl transition-all relative overflow-hidden">
                      <div className="flex items-center gap-6 min-w-0 flex-1">
                        <div className={`p-5 rounded-2xl shadow-xl transition-all ${f.status === 'done' ? 'bg-green-500 text-white' : 'bg-white dark:bg-slate-900 text-emerald-500'}`}>
                          {f.status === 'processing' ? <Loader2 className="animate-spin" size={28} /> : <FileText size={28} />}
                        </div>
                        <div className="text-left min-w-0">
                          <h4 className="text-sm font-black text-slate-900 dark:text-white uppercase truncate tracking-tight mb-1">{f.file.name}</h4>
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{formatSize(f.oldSize)}</span>
                            {f.newSize && (
                              <div className="flex items-center gap-2">
                                <span className="text-slate-300">→</span>
                                <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest bg-emerald-500/10 px-2 py-0.5 rounded-full">
                      ...
                      <div className="flex items-center gap-4 shrink-0">
                        {f.resultUrl && (
                          <a href={f.resultUrl} download={`compressed_${f.file.name}`} className="px-8 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:scale-105 transition-all flex items-center gap-2">
                            <Download size={14} /> Download
                          </a>
                        )}
                        <button onClick={() => removeFile(f.id)} className="p-4 text-slate-400 hover:text-red-500 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700 transition-all hover:scale-110">
                          <X size={20} />
                        </button>
                      </div>
                      {f.status === 'processing' && (
                ...
                <button onClick={() => fileInputRef.current?.click()} className="w-full py-6 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[2rem] flex items-center justify-center gap-4 text-slate-400 hover:text-emerald-500 hover:border-emerald-500 transition-all bg-slate-50/10 group">
                  <div className="p-3 bg-white dark:bg-slate-800 rounded-xl shadow-lg group-hover:scale-110 transition-transform">
                    <FilePlus size={24} />
                  </div>
                  <span className="text-sm font-black uppercase tracking-widest">Add More Documents</span>
                </button>
              </div>
            )}
          </div>
