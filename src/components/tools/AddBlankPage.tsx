...
export default function AddBlankPage({ id: _id }: { id: string }) {
  ...
  const [position, setPosition] = useState<
    "beginning" | "end" | "after" | "after-every"
  >("end");
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("Error adding blank pages.");
    } finally {
      setProcessing(false);
    }
  };
      ...
      <div className="flex flex-col-reverse lg:flex-row-reverse gap-6 items-start">
        ...
        >
          ...
          >
            <span className="flex items-center gap-2">
              <Settings size={20} style={{ color: ACCENT }} /> Page Settings
            </span>
            <ChevronDown
          ...
          <div className={`${showSettings ? "block" : "hidden"} lg:block p-6`}>
            <h3 className="hidden lg:block text-xl font-bold text-slate-900 dark:text-white mb-6 uppercase tracking-tighter text-left">
              Configuration
            </h3>
            ...
            <div className="space-y-6 text-left">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {(
                    ...
                    <button
                      ...
                      disabled={!file}
                      className={`py-2.5 px-4 rounded-xl border-2 text-[11px] font-medium transition-all text-left ${position === p.val ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600" : "border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-indigo-300"}`}
                    >
                      {p.label}
                    </button>
                  ))}
              ...
              {position === "after" && (
                <div className="space-y-3 pt-4 border-t border-slate-50 dark:border-slate-700">
                  ...
                  <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900 rounded-xl p-2 border border-slate-100 dark:border-slate-700">
                    <button
                      onClick={() => setAfterPage((p) => Math.max(1, p - 1))}
                      className="w-8 h-8 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm text-slate-600"
                    >
                      <Minus size={14} />
                    </button>
                    <span className="font-medium text-slate-900 dark:text-white text-xs uppercase tracking-tight">
                      {afterPage}{" "}
                    ...
                    <button
                      onClick={() =>
                        setAfterPage((p) => Math.min(pageCount, p + 1))
                      }
                      className="w-8 h-8 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm text-slate-600"
                    >
                      <Plus size={14} />
              ...
              <div className="space-y-3 pt-4 border-t border-slate-50 dark:border-slate-700">
                ...
                <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900 rounded-xl p-2 border border-slate-100 dark:border-slate-700">
                  ...
                  <span className="font-medium text-slate-900 dark:text-white text-xs uppercase tracking-tight">
                    {blankCount} {blankCount === 1 ? "Page" : "Pages"}
                  </span>
                  <button
                ...
                <div className="grid grid-cols-2 gap-2">
                  {PAGE_SIZES.map((s) => (
                    <button
                      ...
                      disabled={!file}
                      className={`py-2 px-1 rounded-lg border-2 text-[11px] font-medium transition-all ${pageSize === s.value ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600" : "border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-indigo-300"}`}
                    >
                      {s.label}
                    </button>
                  ))}
        ...
        <div className="flex-1 bg-white dark:bg-slate-800 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-12 border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-2xl transition-shadow duration-300 min-h-[500px] flex flex-col w-full">
          ...
          <div className="text-center space-y-4 mb-10">
            <div
              className="inline-flex p-4 rounded-2xl text-white shadow-lg shadow-indigo-500/20"
            ...
            <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 font-medium max-w-md mx-auto leading-relaxed">
              Expand your PDF by inserting high-quality blank templates.
            </p>
          </div>
          ...
          {!file && !processing && (
            ...
            >
              ...
              />
              <div className="p-6 bg-white dark:bg-slate-800 rounded-3xl shadow-sm hover:shadow-xl transition-shadow duration-300 text-indigo-500 mb-6 group-hover:scale-110 transition-transform">
                <Upload size={32} />
              </div>
              <div className="text-lg sm:text-lg sm:text-xl font-medium text-slate-800 dark:text-white mb-1">
                Drop PDF to expand
              </div>
              <p className="text-xs sm:text-sm text-slate-400 font-medium">
                Perfect for notes, printing, or spacers
              </p>
              <button
                className="mt-8 px-10 py-4 rounded-2xl text-white text-sm font-medium uppercase tracking-widest shadow-xl hover:scale-105 transition-all"
                style={{ background: ACCENT_GRADIENT }}
              >
                Choose PDF
              </button>
            ...
            <div className="space-y-8 flex-1 flex flex-col justify-center max-w-2xl mx-auto w-full">
              ...
              <div className="p-8 bg-slate-50 dark:bg-slate-900/50 rounded-[2rem] border border-slate-100 dark:border-slate-700 space-y-6">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-2xl bg-white dark:bg-slate-800 flex items-center justify-center text-indigo-500 shadow-xl border border-slate-50 dark:border-slate-700">
                    <FileText size={32} />
                  </div>
                  <div className="min-w-0">
                    <p className="font-medium text-slate-900 dark:text-white text-xl truncate">
                      {file.name}
                    </p>
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4 w-full">
                  <div className="min-w-0 p-3 sm:p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
                    <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1 truncate">
                      New Total
                    </p>
                    <p className="text-base sm:text-2xl font-medium text-slate-900 dark:text-white tracking-widest truncate">
                      {position === "after-every"
                        ? pageCount * blankCount + pageCount
                        : pageCount + blankCount}
                    </p>
                  </div>
                  <div className="min-w-0 p-3 sm:p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
                    <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1 truncate">
                ...
                <div className="flex items-center gap-3 p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-800/50">
                  ...
                  <p className="text-xs font-medium text-indigo-700 dark:text-indigo-300">
                    Will insert {blankCount}{" "}
                    {blankCount === 1 ? "page" : "pages"}{" "}
                    {position === "beginning"
                      ? "at the very start"
                      : position === "end"
                        ? "at the very end"
          ...
          {result && (
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-10 animate-in zoom-in fade-in duration-500">
              ...
              <div className="space-y-2">
                <h3 className="text-4xl font-bold text-slate-900 dark:text-white uppercase tracking-widest">
                  Document Updated!
                </h3>
                <p className="text-slate-500 font-medium uppercase tracking-widest text-sm">
                  {blankCount} blank page{blankCount !== 1 ? "s" : ""} added
                  successfully
                </p>
              </div>
