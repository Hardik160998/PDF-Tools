        ...
        <div className={`relative flex-1 rounded-2xl border-2 border-dashed transition-all cursor-pointer group ${file ? "border-solid" : ""}`}
            ...
            {file ? (
                ...
                <div className="p-8 flex flex-col items-center gap-3 pointer-events-none">
                    ...
                    <h3 className="text-xl sm:text-2xl font-black text-slate-800 dark:text-white mb-2 tracking-tight text-center">PDF {label}</h3>
                                    ...
                                    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                                        ...
                                        <div className="max-h-96 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-700/50">
                                            {cur.lineDiffs.length === 0 ? (
                                                ...
                                                <div key={idx} className={`flex gap-3 px-4 py-2 text-sm font-medium leading-relaxed ${line.type === "added" ? "bg-green-50 dark:bg-green-900/20" :
                                                        line.type === "removed" ? "bg-red-50 dark:bg-red-900/20" :
                                                            "bg-white dark:bg-slate-800"
                                                    }`}>
                                                    <span className={`shrink-0 w-5 text-center font-medium text-xs mt-0.5 ${line.type === "added" ? "text-green-500" :
                                                            line.type === "removed" ? "text-red-500" :
                                                                "text-slate-300 dark:text-slate-600"
                                                        }`}>
                                                        {line.type === "added" ? "+" : line.type === "removed" ? "−" : ""}
                                                    </span>
                                                    <span className={`flex-1 ${line.type === "added" ? "text-green-800 dark:text-green-300" :
