...
import {
  ...
  Sparkles, Lock, Trash2, Smartphone, Rocket, Plus, Shield, RefreshCw
} from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto py-2 sm:py-8 px-2 sm:px-6 ">
      <div className="flex flex-col lg:flex-row-reverse gap-8 items-start">
        ...
        <div className={`w-full lg:w-[320px] bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-shadow duration-300 overflow-hidden h-fit lg:sticky lg:top-4 flex-shrink-0 ${files.length === 0 || !isSplit ? 'hidden' : ''}`}>
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-tighter">Configuration</div>
        ...
        <div className="flex-1 w-full space-y-4 sm:space-y-6">
          <div className={`bg-white dark:bg-slate-900 rounded-3xl sm:rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-shadow duration-300 p-5 sm:p-10 min-h-[500px] flex flex-col relative overflow-hidden ${files.length === 0 || !isSplit ? 'max-w-4xl mx-auto w-full' : 'w-full'}`}>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className={`p-6 sm:p-10 rounded-3xl sm:rounded-[2.5rem] border flex flex-col lg:flex-row items-center lg:items-center justify-between gap-8 mb-8 sm:mb-6 text-center lg:text-left ${isSplit ? 'bg-violet-50 dark:bg-violet-500/5 border-violet-100 dark:border-violet-500/20' : 'bg-green-50 dark:bg-green-500/5 border-green-100 dark:border-green-500/20'}`}>
                  <div className="flex flex-col sm:flex-row items-center gap-6">
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-slate-900 dark:text-white uppercase tracking-widest leading-none mb-1">{isSplit ? 'Processing Complete' : 'Merge Ready'}</h2>
                  ...
                  {mergedResult && (
                    <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
                      ...
                      <button onClick={handleReset} className={`px-10 py-5 bg-white dark:bg-slate-800 border-2 rounded-2xl font-medium text-sm uppercase tracking-widest shadow-xl hover:scale-105 transition-all flex items-center justify-center gap-3 w-full sm:w-auto ${isSplit ? 'text-violet-600 dark:text-violet-400 border-violet-100 dark:border-violet-500/20' : 'text-slate-600 dark:text-slate-400 border-slate-100 dark:border-slate-700'}`}>
...
function SortableFile({ f, i, isSplit, removeFile }: { f: ProcessedFile; i: number; isSplit: boolean; removeFile: (id: string) => void }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex flex-row items-center justify-between p-3 sm:p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 transition-all group gap-2 sm:gap-4 w-full"
    >
