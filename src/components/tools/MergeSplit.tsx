        ...
        <div className="w-full lg:w-[320px] bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden h-fit lg:sticky lg:top-4 flex-shrink-0">
          ...
          <div className={`${showSettings ? 'block' : 'hidden'} lg:block p-6`}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-outfit hidden lg:block text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Configuration</h3>
              <button onClick={handleReset} className="font-outfit text-[11px] font-medium uppercase tracking-widest text-red-500 hover:text-red-600 transition-colors">Reset</button>
            </div>
              ...
              {isSplit && (
                <div className="space-y-4 animate-in slide-in-from-top-4 duration-300">
                  <div className="space-y-2">
                    <span className="font-outfit text-[11px] font-medium text-slate-400 uppercase tracking-widest text-left block">Split Mode</span>
                    <div className="flex bg-slate-50 dark:bg-slate-800 p-1.5 rounded-2xl border border-slate-100 dark:border-slate-800">
                      {(['parts', 'extract'] as const).map(mode => (
                        <button key={mode} onClick={() => setSplitMode(mode)}
                          className={`flex-1 py-3 font-outfit text-[11px] font-medium uppercase tracking-widest rounded-xl transition-all ${splitMode === mode ? 'bg-white dark:bg-slate-900 shadow-xl text-violet-600' : 'text-slate-400 hover:text-slate-600'}`}>
                  ...
                  {splitMode === 'parts' && (
                    <div className="space-y-2">
                      <span className="font-outfit text-[11px] font-medium text-slate-400 uppercase tracking-widest text-left block">Target Parts</span>
                      <div className="grid grid-cols-3 gap-2">
                        {[2, 3, 4].map(num => (
                          <button key={num} onClick={() => setSplitParts(num)}
                            className={`py-3 rounded-xl border-2 font-outfit font-medium text-xs transition-all ${splitParts === num ? 'border-violet-500 bg-violet-50 dark:bg-violet-500/10 text-violet-600' : 'border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-400'}`}>
                            {num}
                          </button>
                        ))}
                      </div>
                ...
                {isSplit && splitResults.length > 0 ? (
                      ...
                      <div key={i} className="bg-slate-50 dark:bg-slate-800/50 rounded-3xl sm:rounded-[2.5rem] p-4 sm:p-6 border border-slate-100 dark:border-slate-800 flex flex-col gap-4 sm:gap-6 group hover:shadow-2xl transition-all relative">
                        <div className="aspect-square rounded-[2rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center relative overflow-hidden group-hover:border-violet-500/50 transition-colors">
                          ...
                          <div className="absolute bottom-4 left-0 right-0 px-4">
                            <p className="font-outfit text-[11px] font-medium text-slate-400 uppercase tracking-widest text-center">{res.pageCount || '?'} Pages</p>
                          </div>
                  ...
                  <>
                    <div className="flex items-center justify-between px-2 mb-6">
                      ...
                      <button onClick={() => fileInputRef.current?.click()} className="font-outfit text-[11px] font-medium uppercase tracking-widest text-orange-500 hover:opacity-80">
                        Add More
                      </button>
                    </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 shrink-0">
         {f.resultUrl && (
           <a href={f.resultUrl} download={f.resultName} className="px-5 py-2.5 bg-orange-500 text-white rounded-xl font-outfit text-[11px] font-medium uppercase tracking-widest shadow-lg hover:scale-105 transition-all flex items-center gap-2">
