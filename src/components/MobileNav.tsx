               ...
               {filtered ? (
                 filtered.length > 0 ? (
                   filtered.map(({ href, label, icon: Icon }) => {
                     ...
                     return (
                       ...
                       >
                         <span
                           className="mob-drawer-item-icon"
                 ...
                 SECTIONS.map((section) => (
                   <div key={section.label} className="mob-drawer-section">
                     <div
                       className="mob-drawer-section-label"
               ...
               {loading ? (
                 ...
                 <div className="flex flex-col gap-3 w-full bg-slate-50 dark:bg-slate-800/40 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                   <div className="flex items-center gap-2.5">
                     <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-500 to-pink-500 flex items-center justify-center text-xs font-bold text-white shadow-md">
                       {profile?.full_name
                         ?.split(" ")
                         .map((n) => n[0])
                         .slice(0, 2)
                   ...
                   <div className="grid grid-cols-2 gap-2 mt-1">
                     <Link
                       href="/profile"
                       onClick={close}
                       className="flex items-center justify-center gap-1.5 py-2 px-3 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-700/60 rounded-xl text-[10px] font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all text-center"
                     >
                       My Profile
                     </Link>
                     <button
                       onClick={handleLogout}
                       className="flex items-center justify-center gap-1.5 py-2 px-3 bg-rose-500 hover:bg-rose-600 text-white rounded-xl text-[10px] font-bold shadow-md shadow-rose-500/10 transition-all cursor-pointer text-center"
                     >
                       Log Out
                     </button>
                   </div>
                 ...
                 <div className="grid grid-cols-2 gap-3 w-full">
                   <Link
                     href="/login"
                     onClick={close}
                     className="flex items-center justify-center py-3 px-4 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all text-center"
                   >
                     Login
                   </Link>
                   <Link
                     href="/signup"
                     onClick={close}
                     className="flex items-center justify-center py-3 px-4 bg-[#ef4444] hover:bg-[#dc2626] text-white rounded-xl text-xs font-bold shadow-lg shadow-red-500/20 transition-all text-center"
                   >
                     Sign Up
                   </Link>
                 </div>
